...
path = Path("app/commerce-widgets-v14.tsx")
...
confirm_pattern = re.compile(r"  function confirmOrder\(\) \{\n.*?\n  \}\n\n  return <>", re.S)
confirm_replacement = '''  function confirmOrder() {
  ...
  return <>'''
text, count = confirm_pattern.subn(lambda _match: confirm_replacement, text, count=1)
if count != 1:
    raise SystemExit(f"confirm order: expected 1 match, found {count}")
...
print("E-book checkout V17C applied")
